...
if config.DEMO_MODE and config.DEBUG_MODE:
    from Lab5.RL import demo_config_td_lambda as model_config
else:
...
LAMBDA: float = model_config.LAMBDA                         # td lambda decay factor
...
def act(y_prev, x_prev, yy_prev, xx_prev, acc_vector, env_data, is_deterministic=(False),
    ...
    if env_data[yy_prime][x_prime] == STATE_TERMINAL:
        gam_end = True

    return y_prime, x_prime, yy_prime, xx_prime, game_end
...
def perform_td_lambda(env_data, reset_after_crash: bool = False, reward: float = 0.0, epochs=(EPOCHS), episodes=EPISODES):
    rows = len(env_data)
    cols = len(env_data[0])
    ...
    for epoch in range(0, epochs):
        ...
        for episode in range(0, episodes):
            ...
            y_prime, x_prime, yy_prime, xx_prime, game_end = act(y, x, yy, xx, action_space[action], env_data, reset_after_crash=reset_after_crash)
            reward = -1
            if game_end:
                reward = 100
            epoch_reward += reward
            ...
            td_error = reward + GAMMA * q_value_max - q_value_prev
            key: str = f"{y}+{x}+{yy}+{xx}"
            eligibility[key][action] += 1
            for s in eligibility.keys():
                # print(f"s: {s}")
                for a in eligibility[s].keys():
                    # print(f"a: {a}")
                    s_keys = s.split("+")
                    # print(s_keys)
                    ys, xs, yys, xxs = int(s_keys[0]), int(s_keys[1]), int(s_keys[2]), int(s_keys[3])
                    # print(f"s_keys: {ys}, {xs}, {yys}, {xxs}")
                    q_value[ys][xs][yys][xxs][a] += NU * td_error * eligibility[s][a]
...
def initialize_race(env_data, policy, reset_after_crash, update_steps=UPDATE_STEPS):
    ...
    for index_i in range(0, update_steps):
        ...
        y, x, yy, xx, game_end = act(y, x, yy, xx, action, env_data, reset_after_crash=reset_after_crash)
...
def train_td_lamda_algorithm(explainability: bool = False):
    """
    Driver function for the Q-learning algorithm
    ...
    while (epochs < EPOCH_THRESHOLD):
        ...
        policy = perform_td_lambda(racetrack, reset_after_crash=RESET_AFTER_CRASH, epochs=epochs)
        ...
        stats_file += '_'
        stats_file += f'td_lambda_epoch'
        stats_file += f'{epochs}_crash'
        stats_file += f'{RESET_AFTER_CRASH}_{LAMBDA}_stats.txt'
        ...
        output_file.write(f'td lambda summmary stats:\n')
        output_file.write(f'_______________________________________________________________\n')
        ...
        output_file.write(
            f'cost & average number of steps vehicle took before finish line: {step_count / LAP_COUNT} steps')

        # for more QC, the explainability flag will allow the model to save all SARSA tuples
        if explainability:
            controls_log = f'output/{track_name}'
            controls_log += '_'
            controls_log += f'td_lambda_epoch'
