# value_iteration_iteration.py
# Kordel France
########################################################################################################################
# This file establishes the specification for the value iteration algorithm for reinforcement learning.
########################################################################################################################
...
if config.DEMO_MODE and config.DEBUG_MODE:
    from Lab5.RL import demo_config_value_iteration as model_config
else:
...
def perform_value_iteration(env_data, is_crash_severe: bool = False, reward=(0.0), epochs=EPOCHS):
    rows = len(env_data)
    cols = len(env_data[0])

    # create and initialize value table
    value_table = [[[[random() for _ in state_space_velocity] for _ in state_space_velocity] for _ in (line)] for line
                   in env_data]
    for y in range(0, rows):
        for x in range(0, cols):
            if env_data[y][x] == STATE_TERMINAL:
                for yy in state_space_velocity:
                    for xx in state_space_velocity:
                        value_table[y][x][yy][xx] = reward

    # get initial q_value
    q_value = [[[[[random() for _ in action_space] for _ in state_space_velocity] for _ in (state_space_velocity)] for _ in
          line] for line in env_data]
    ...
    # begin updating q-values
    for epoch in range(0, epochs):
        value_table_prev = deepcopy(value_table)
        epsilon = 0.0
        for y in range(0, rows):
            for x in range(0, cols):
                for yy in state_space_velocity:
                    for xx in state_space_velocity:
                        ...
                        for action_i, action in enumerate(action_space):
                            ...
                            # print(f'action_i: {action_i}\taction: {action}')
                            y_prime, x_prime, yy_prime, xx_prime = act(y, x, yy, xx, action, env_data,
                                                                       is_deterministic=True,
                                                                       is_crash_severe=is_crash_severe)
                            #### BREAKPOINT 3 ###
                            value_updated_state_action_success = value_table_prev[y_prime][x_prime][yy_prime][xx_prime]

                            init_acc_vector = (0, 0)
                            y_prime, x_prime, yy_prime, xx_prime = act(y, x, yy, xx, init_acc_vector, env_data,
                                                                       is_deterministic=True,
                                                                       is_crash_severe=is_crash_severe)
                            #### BREAKPOINT 4 ###
                            value_updated_state_acton_fail = value_table_prev[y_prime][x_prime][yy_prime][xx_prime]

                            # new state is product of probabilities of accelerator passing and failing
                            state_prime = (ACCELERATION_RATE * value_updated_state_action_success) + (
                                        ACCELERATION_MISFIRE_RATE * (value_updated_state_acton_fail))

                            #### BREAKPOINT 5 ###
                            q_value[y][x][yy][xx][action_i] = reward_prime + (GAMMA * state_prime)

                        q_value_prime = q_value[y][x][yy][xx]
                        action_highest_q = np.argmax(q_value_prime)
                        # update value table - V(s)

                        #### BREAKPOINT 6 ###
                        value_table[y][x][yy][xx] = q_value[y][x][yy][xx][action_highest_q]
        ...
        epsilon0 = abs(value_table[y][x][yy][xx] - value_table_prev[y][x][yy][xx])
        epsilon1 = max([epsilon0 for yy in state_space_velocity])
        epsilon2 = max([epsilon1 for xx in state_space_velocity])
        epsilon3 = max([epsilon2 for y in range(0, rows)])
        epsilon = max([epsilon3 for x in range(0, cols)])

        if epsilon < Q_STABILITY_ERROR:
            policy = update_policy_given_q(cols, rows, state_space_velocity, q_value, action_space)
            return (policy)
...
def train_value_iteration_algorithm(explainability:bool=False):
    """
    Driver function for the value iteration algorithm
    :param explainability: bool - a flag that allows all agent SA tuples to be saved for debugging and QC
    """
    # reset the spec file in case there was a model run before with a different config
    reset_hyperparameters()
    print('starting racetrack RL agent for Value Iteration algorithm')
    epochs = EPOCHS
    epoch_list: list = []
    step_list: list = []
    ...
    while (epochs < EPOCH_THRESHOLD):
        ...
        policy = perform_value_iteration(racetrack, is_crash_severe=RESET_AFTER_CRASH, epochs=epochs)
        ...
        output_file.write(f'va summmary stats:\n')
        output_file.write(f'_______________________________________________________________\n')
...
if config.DEMO_MODE:
    train_value_iteration_algorithm()
